* Open log file
local dofile_name "referee_point5"
...
preserve
	keep cntryid tot_paid_year
	duplicates drop cntryid, force
	sum tot_paid_year, detail
	gen pl_group = tot_paid_year >= r(p50)

	qui count if pl_group
	assert float(r(N)) * 2 == _N
	keep cntryid pl_group
	
	tempfile tmp
	save `tmp', replace
restore
merge m:1 cntryid using `tmp'
		...
		* CBA
		...
		forvalues i = 2(1)2 {
			...
			foreach k in 0 1 {
				forvalues j = 5(1)5 {
					local inst "${inst`j'}"
					eststo: `model`i'' ${indepvar} ///
						if `cond`i'' & pl_group == `k', vce(cluster cluster_`skill') `het`i''
					qui estadd_spec, spec(`j')
					qui estadd local ncntry = strofreal(e(N_clust) / 4, "%2.0f")
				}
				eststo: reg work`skill' ${indepvar} ///
					if `cond`i'' & work == 1 & pl_group == `k', vce(cluster cluster_`skill')
				qui estadd_spec, spec(5)
				qui estadd local ncntry = strofreal(e(N_clust) / 4, "%2.0f")
			}
			local coef_lab_lit "Literacy"
			local coef_lab_num "Numeracy"
			local texfile "${path_table}/tex/sub/mismatch_`skill'`tag'`tag`i''_sub_refp5.tex"
			...
			# d;
				...
				mgroups("Full sample" "Employed" "Full sample" "Employed", pattern(1 1 1 1)
